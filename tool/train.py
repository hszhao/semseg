...
def get_logger():
    ...
    logger.log_metrics = partial(log_metrics, logger)
    return logger
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == "psp":
        ...
        modules_new = [model.up1, model.up2, model.up3, model.up4, model.up5]
    ...
    for module in modules_new:
        params_list.append(dict(params=module.parameters(), lr=args.base_lr * head_lr_factor))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, model, optimizer, epoch, head_lr_factor)
        if main_process():
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            # if epoch_log / args.save_freq > 5:
            #    deletename = args.save_path + '/train_epoch_' + str(epoch_log - args.save_freq * 5) + '.pth'
...
def train(train_loader, model, optimizer, epoch, head_lr_factor=10):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if hasattr(args, "lr_schedule") and args.lr_schedule == "cosine":
            current_lr = cosine_learning_rate(args.base_lr, current_iter, max_iter)
        else:
            current_lr = poly_learning_rate(args.base_lr, current_iter, max_iter, power=args.power)
        for index in range(0, args.index_split):
            optimizer.param_groups[index]["lr"] = current_lr
        for index in range(args.index_split, len(optimizer.param_groups)):
            optimizer.param_groups[index]["lr"] = current_lr * head_lr_factor
