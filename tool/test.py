...
def main():
    ...
    if args.calc_results:
        keys, data = cal_acc(test_data.data_list, gray_folder, args.classes, names)

        with open(args.result_path, "w") as f:  # You will need 'wb' mode in Python 2.x
            w = csv.DictWriter(f, keys)
            w.writeheader()
            w.writerows([dict(zip(keys, row)) for row in data])


def net_process(model, image, mean, std=None, flip=True, use_cuda=True):
...
def scale_process(
    ...
    for index_h in range(0, grid_h):
        for index_w in range(0, grid_w):
            ...
            prediction_crop[s_h:e_h, s_w:e_w, :] += net_process(model, image_crop, mean, std, use_cuda)
...
def cal_acc(data_list, pred_folder, classes, names):
    ...
    keys = ["class", "IoU", "Acc"]
    mious = [val.item() for val in (mIoU, *iou_class)]
    accs = [val.item() for val in (mAcc, *accuracy_class)]
    data = list(zip(["mean"] + names, mious, accs))
    return keys, data
